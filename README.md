...
Zoom VTTがなく、Whisper+pyannote の主VTTだけがある場合は、**単一VTTモード**で実行する。補助VTTの存在チェックとオフセット検出をスキップし、主VTTのみをLLMで整形・補正する。

**CLIで指定する場合:**

```powershell
py -V:3.13 main.py --job "C:\Users\user\dev\TranscriptMergerJobs\2026-03-07_定例会議" --single-vtt
```
...
単一VTTモードでは `source_zoom_vtt` は出力JSONで `null` となり、オフセットレポートの `method` は `single_vtt` になる。

### 特定チャンクのみ再処理したい場合

一部のチャンクだけIDバリデーション失敗で落ちた場合、そのチャンク番号を指定して再処理できる。既存の完了済みチャンクはスキップされ、指定したチャンクのみ再実行される。`--clean` と併用はできない。

```powershell
# チャンク12のみ再処理
py -V:3.13 main.py --job "C:\Users\user\dev\TranscriptMergerJobs\2026-03-07_定例会議" --chunk 12

# チャンク12と31を再処理
py -V:3.13 main.py --job "C:\Users\user\dev\TranscriptMergerJobs\2026-03-07_定例会議" --chunk 12 --chunk 31
```

再処理後、全チャンクが完了していれば Step 6 で最終出力が生成される。
