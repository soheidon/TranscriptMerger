...
| v3.3 | 2026-03-10 | 部分再処理オプション（--chunk）追加。Zoom VTTの参照ラベルを [V001] から [ZOOM_REF_001] に変更し、id/source_ids への出力禁止をプロンプトで明示。IDバリデーション不合格時のリトライで、前回の誤り内容をプロンプトに反映するよう変更 |
...
[ZOOM_REF_001] こんにちは、今日は...
[ZOOM_REF_002] ありがとうございます...
...
...
- Zoom VTT の行頭ラベルは **参照用** であり、`[ZOOM_REF_001]` 形式で提示する。これらは JSON の `id` および `source_ids` に**絶対に出力してはならない**。プロンプトで明示する。
...
| Zoom参照ラベルの出力 | Zoom VTTの参照ラベル（`ZOOM_REF_001` 等）を `id` や `source_ids` に含めてはならない。これらは入力の行頭表示用であり、出力に使用するIDではない |
...
**リトライ時のプロンプト補強:** リトライ時には、前回のバリデーション不合格理由（例: 「未知ID: V007 が出力に含まれている」「ID重複: U000012 が複数utteranceに出現」）をプロンプト冒頭に追記する。これにより同一の誤り（Zoom参照ラベルの混入・ID欠損・ID重複など）を繰り返しにくくする。
...
特定チャンクのみを再処理するためのCLIオプションを用意する。

| オプション | 説明 |
|-----------|------|
| `--chunk N` | 再処理対象のチャンク番号。複数指定可（例: `--chunk 12 --chunk 31`） |

- 指定したチャンク番号に対応するチャンクのみ Step 5 を実行する。それ以外のチャンクは既存の `temp_chunk_XXX.json` / `meta.json` をそのまま利用する（スキップしない場合は再計算しない）。
- Step 1〜4 は従来どおり実行する（オフセット・ID・チャンク分割は全件計算）。
- Step 6 は**全チャンク**の完了状態（`temp/` 内の meta の status）を見て strict / best_effort を判定し、最終出力を生成する。
- `--clean` と `--chunk` は**併用禁止**とする。部分再処理では既存の完了結果を残す必要があるため。
- 存在しないチャンク番号を指定した場合はエラーで終了する。
