...
11. 自己紹介（「〜の江口です」「〜に所属しています」等）は、名前の本人に割り当てること。他の話者に吸収しないこと
12. 3人目の話者が登場した直後は、挨拶と自己紹介が連続する。この区間では「誰が誰に向かって話しているか」を会話の流れから判断し、機械的に前話者に吸収しないこと
13. 質問に対する「僕？」「え？」「何ですか？」のような短い確認は、質問を受けた側（被質問者）の発話として割り当てること
...
5. AB_MISMATCHの区間で主VTTのテキストが明らかに壊れている場合（意味をなさない文字列が含まれる場合）、Zoom VTTのテキストを優先して採用すること。両方壊れている場合は、文脈から推定可能な範囲で整形し、uncertain=true + edit_type=VTT_SUPPLEMENT を設定すること
6. 意味不明な文字列（「まちろん」「マグティノム」「コックする」等）がASR出力に含まれる場合、そのまま残さず、文脈から推定するか、推定できなければ該当部分を省略してuncertain=trueを付けること
7. AB_MISMATCHが連続する区間で、個別の短い発話を乱立させず、ひとまとまりの発話として保守的にまとめること
...
【テキスト整形ルール】
1. 意味を持たないフィラー語（えー、えっと、あのー、あー、うーん、まあ、そのー、なんか等）は削除すること。文頭のフィラー（「であの」「えっと」「あのなんていうかまあなんとかこう」等）は特に削除すること。前回の出力で削除できていたフィラーを復活させないこと
...
10. 独り言・考え中の発話（相手が席を外している間など）でも、読み物として意味が通るよう最低限の整形はすること
11. 前後の会話の流れに全く合わない孤立した短い発話（「ではないです」「ありがとうございました」等が脈絡なく挿入される場合）は、ASRの誤認識と判断し、category=BACKCHANNEL + uncertain=true + uncertain_reason=LOW_CONFIDENCE を設定すること。LLMが内容として採用してはならない
12. 同一文書内で同じ語の表記を統一すること（例:「お疲れさまです」と「お疲れ様です」が混在→どちらかに統一）
